...
tidas::mpi_volume::mpi_volume(mpi_volume const & other, std::string const & filter,
                              backend_path const & loc) {
    copy(other.comm(), other, filter, loc);
}

tidas::mpi_volume::mpi_volume(MPI_Comm comm, mpi_volume const & other,
...
string tidas::mpi_volume::path_make(string const & path) const {
    ...
    if (rank_ == 0) {
        if (fs_stat(path.c_str()) >= 0) {
            ostringstream o;
            o << "cannot create mpi_volume \"" << path <<
...
string tidas::mpi_volume::path_get(string const & path) const {
    ...
    if (rank_ == 0) {
        if (fs_stat(path.c_str()) == 0) {
            ostringstream o;
            o << "cannot get mpi_volume path \"" << path <<
...
void tidas::mpi_volume::open() {
    ...
    if (histsize > 0) {
        ostringstream o;
        o << "process " << rank_ << " in mpi_volume has " << histsize <<
            " outstanding transactions that would be lost during sync";
        TIDAS_THROW(o.str().c_str());
    }
...
void tidas::mpi_volume::close() {
    ...
    //     if ( rank_ == p ) {
    //         std::cerr << "DBG: mpi_volume close proc " << p << " local:" <<
    // std::endl;
    //         for ( auto const & h : hist ) {
    //             h.print ( std::cerr );
    //         }
    ...
    if (rank_ == 0) {
        for (size_t i = 0; i < nproc_; ++i) {
            // std::cerr << "DBG: mpi_volume close replaying from proc " << i << ":" <<
            // std::endl;
...
void tidas::mpi_volume::index_setup() {
    ...
    // std::cout << "proc " << rank_ << ", master = " << masterdb_.get() << " local = "
    // << localdb_.get() << std::endl;

    return;
}

void tidas::mpi_volume::copy(MPI_Comm comm, mpi_volume const & other,
                             string const & filter, backend_path const & loc) {
    ...
    if (histsize > 0) {
        ...
        // }
        ostringstream o;
        o << "process " << rank_ << " in mpi_volume copy source has " << histsize <<
...
void tidas::mpi_volume::link(std::string const & path, link_type const & ltype,
    ...
    if (rank_ == 0) {
        ...
        if (fs_stat(lpath.c_str()) >= 0) {
            ostringstream o;
            o << "cannot create volume link \"" << lpath <<
                "\", a file or directory already exists";
            TIDAS_THROW(o.str().c_str());
        }
...
void tidas::mpi_volume::duplicate(std::string const & path, backend_type type,
...
void tidas::mpi_volume::wipe(string const & filter) const {
    if (loc_.type != backend_type::none) {
        if (loc_.mode == access_mode::write) {} else {
