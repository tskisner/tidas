...
class block {
    public:
        ...
        /// (**Internal**) Create a copy of a block, with optional selection and new
        // location.
        block(block const & other, std::string const & filter,
        ...
        void copy(block const & other, std::string const & filter,
                  backend_path const & loc);
        ...
        void exec(P & op, exec_order order) {
            ...
            if (block_data_.size() == 0) {
                ...
                for (std::map <std::string, block>::const_iterator it =
                         block_data_.begin();
                     it != block_data_.end(); ++it) {
    ...
    private:
        ...
        backend_path group_loc(backend_path const & loc,
                               std::string const & name) const;

        backend_path intervals_loc(backend_path const & loc,
                                   std::string const & name) const;

        backend_path block_loc(backend_path const & loc,
                               std::string const & name) const;
