...
class group_backend {
    public:
        ...
        virtual void read(backend_path const & loc, index_type & nsamp,
                          time_type & start, time_type & stop, std::map <data_type,
                                                                         size_t> & counts)
            = 0;

        virtual void write(backend_path const & loc, index_type const & nsamp,
                           std::map <data_type, size_t> const & counts) const = 0;

        virtual void resize(backend_path const & loc, index_type const & nsamp) = 0;

        virtual void update_range(backend_path const & loc, time_type const & start,
                                  time_type const & stop) = 0;
        ...
        virtual void read_field(backend_path const & loc,
                                std::string const & field_name, size_t type_indx,
                                index_type offset, index_type ndata,
                                int8_t * data) const = 0;
        virtual void write_field(backend_path const & loc,
                                 std::string const & field_name, size_t type_indx,
                                 index_type offset, index_type ndata,
                                 int8_t const * data) = 0;
        ...
        virtual void write_field(backend_path const & loc,
                                 std::string const & field_name, size_t type_indx,
                                 index_type offset, index_type ndata,
                                 uint16_t const * data) = 0;

        virtual void read_field(backend_path const & loc,
                                std::string const & field_name, size_t type_indx,
                                index_type offset, index_type ndata,
                                int32_t * data) const = 0;
        virtual void write_field(backend_path const & loc,
                                 std::string const & field_name, size_t type_indx,
                                 index_type offset, index_type ndata,
        ...
        virtual void read_field(backend_path const & loc,
                                std::string const & field_name, size_t type_indx,
                                index_type offset, index_type ndata,
                                int64_t * data) const = 0;
        virtual void write_field(backend_path const & loc,
                                 std::string const & field_name, size_t type_indx,
                                 index_type offset, index_type ndata,
                                 int64_t const * data) = 0;
...
class group_backend_hdf5 : public group_backend {
    public:
        ...
        void read(backend_path const & loc, index_type & nsamp, time_type & start,
                  time_type & stop, std::map <data_type, size_t> & counts);
        void write(backend_path const & loc, index_type const & nsamp,
                   std::map <data_type, size_t> const & counts) const;

        void resize(backend_path const & loc, index_type const & nsamp);
        void update_range(backend_path const & loc, time_type const & start,
        ...
        void read_field(backend_path const & loc, std::string const & field_name,
                        size_t type_indx, index_type offset, index_type ndata,
                        int8_t * data) const;
        void write_field(backend_path const & loc, std::string const & field_name,
                         size_t type_indx, index_type offset, index_type ndata,
                         int8_t const * data);
...
class group {
    public:
        ...
        void copy(group const & other, std::string const & filter,
                  backend_path const & loc);
        ...
        template <class T>
        void read_field(std::string const & field_name, index_type offset,
                        index_type ndata, T * data) const {
            field check = schm_.field_get(field_name);
            if ((check.name != field_name) && (field_name != group_time_field)) {
                std::ostringstream o;
                o << "cannot read non-existent field " << field_name <<
                    " from group " << loc_.path << "/" << loc_.name;
                TIDAS_THROW(o.str().c_str());
            }
            if (offset + ndata > size_) {
                std::ostringstream o;
                o << "cannot read field " << field_name << ", samples " << offset <<
                    " - " << (offset + ndata - 1) << " from group " << loc_.name <<
                    " (" <<
                    size_ << " samples)";
                TIDAS_THROW(o.str().c_str());
            }
            if (loc_.type != backend_type::none) {
                backend_->read_field(loc_, field_name, type_indx_.at(
                                         field_name), offset, ndata, data);
            } else {
        ...
        void read_field_astype(std::string const & field_name,
                               index_type offset, index_type ndata,
                               tidas::data_type type,
                               void * data) const;

        /// Write a range of data to a field.  Data is provided at the
        /// bare memory address specified.
        template <class T>
        void write_field(std::string const & field_name, index_type offset,
                         index_type ndata, T const * data) {
            field check = schm_.field_get(field_name);
            if ((check.name != field_name) && (field_name != group_time_field)) {
                std::ostringstream o;
                o << "cannot write non-existent field " << field_name <<
                    " from group " << loc_.path << "/" << loc_.name;
                TIDAS_THROW(o.str().c_str());
            }
            if (offset + ndata > size_) {
                std::ostringstream o;
                o << "cannot write field " << field_name << ", samples " << offset <<
                    " - " << (offset + ndata - 1) << " to group " << loc_.name <<
                    " (" <<
                    size_ << " samples)";
                TIDAS_THROW(o.str().c_str());
            }
            if (loc_.type != backend_type::none) {
                backend_->write_field(loc_, field_name, type_indx_.at(
                                          field_name), offset, ndata, data);
            } else {
