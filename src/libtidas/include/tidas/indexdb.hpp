...
class indexdb {
    public:
        ...
        virtual void add_dict(backend_path loc, std::map <std::string,
                                                          std::string> const & data,
                              std::map <std::string,
                                        data_type> const & types)
            = 0;
        virtual void update_dict(backend_path loc, std::map <std::string,
                                                             std::string> const & data,
                                 std::map <std::string,
                                           data_type> const & types)
            = 0;
        virtual void del_dict(backend_path loc) = 0;
        virtual bool query_dict(backend_path loc, std::map <std::string,
                                                            std::string> & data,
                                std::map <std::string,
                                          data_type> & types)
            = 0;
        ...
        virtual void add_group(backend_path loc, index_type const & nsamp,
                               time_type const & start, time_type const & stop,
                               std::map <data_type, size_t> const & counts) = 0;
        virtual void update_group(backend_path loc, index_type const & nsamp,
                                  time_type const & start, time_type const & stop,
                                  std::map <data_type, size_t> const & counts) = 0;
        virtual void del_group(backend_path loc) = 0;
        virtual bool query_group(backend_path loc, index_type & nsamp,
                                 time_type & start, time_type & stop,
                                 std::map <data_type, size_t> & counts) = 0;
        ...
        virtual bool query_block(backend_path loc,
                                 std::vector <std::string> & child_blocks,
                                 std::vector <std::string> & child_groups,
                                 std::vector <std::string> & child_intervals) = 0;
...
class indexdb_sql : public indexdb {
    public:
        ...
        indexdb_sql(std::string const & path, std::string const & volpath,
                    access_mode mode);

        void copy(indexdb_sql const & other);

        void add_dict(backend_path loc, std::map <std::string,
                                                  std::string> const & data,
                      std::map <std::string,
                                data_type> const & types);
        void update_dict(backend_path loc, std::map <std::string,
                                                     std::string> const & data,
                         std::map <std::string,
                                   data_type> const & types);
        void del_dict(backend_path loc);
        bool query_dict(backend_path loc, std::map <std::string, std::string> & data,
                        std::map <std::string, data_type> & types);
        ...
        void add_group(backend_path loc, index_type const & nsamp,
                       time_type const & start, time_type const & stop,
                       std::map <data_type, size_t> const & counts);
        void update_group(backend_path loc, index_type const & nsamp,
                          time_type const & start, time_type const & stop,
                          std::map <data_type, size_t> const & counts);
        void del_group(backend_path loc);
        bool query_group(backend_path loc, index_type & nsamp, time_type & start,
        ...
        bool query_block(backend_path loc, std::vector <std::string> & child_blocks,
                         std::vector <std::string> & child_groups,
                         std::vector <std::string> & child_intervals);

        void commit(std::deque <indexdb_transaction> const & trans);

        void tree(backend_path root, std::string const & filter,
    ...
    private:
        ...
        void tree_node(backend_path loc, std::string const & filter,
                       std::deque <indexdb_transaction> & trans);

        void ops_begin();
        void ops_end();

        void ops_dict(backend_path loc, indexdb_op op, std::map <std::string,
                                                                 std::string> const & data, std::map <std::string,
                                                                                                      data_type> const & types);

        void ops_schema(backend_path loc, indexdb_op op, field_list const & fields);

        void ops_group(backend_path loc, indexdb_op op, index_type const & nsamp,
                       time_type const & start, time_type const & stop,
                       std::map <data_type, size_t> const & counts);
...
#endif // ifndef TIDAS_INDEXDB_HPP
