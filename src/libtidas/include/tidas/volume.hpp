...
class volume {
    public:
        ...
        volume(std::string const & path, backend_type type, compression_type comp,
               std::map <std::string, std::string> extra = std::map <std::string,
                                                                     std::string> ());
        ...
        volume(volume const & other, std::string const & filter,
               backend_path const & loc);

        // metadata ops

        /// (**Internal**) Create a copy of a volume, with optional selection and new
        // location.
        void copy(volume const & other, std::string const & filter,
                  backend_path const & loc);
