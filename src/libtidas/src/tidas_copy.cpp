...
void tidas::data_copy(intervals const & in, intervals & out) {
    ...
    if ((in.location().type == backend_type::none) ||
        (out.location().type == backend_type::none)) {
        TIDAS_THROW("cannot do data_copy between intervals with unassigned backends");
    }

    if (in.size() != out.size()) {
        ostringstream o;
        o << "cannot data_copy between input intervals of size " << in.size() <<
            " and output intervals of size " << out.size();
...
void tidas::data_copy(group const & in, group & out) {
    ...
    if (in.size() != out.size()) {
        ostringstream o;
        o << "cannot data_copy between input group of size " << in.size() <<
            " and output group of size " << out.size();
        TIDAS_THROW(o.str().c_str());
    }
