...
tidas::group::group(group const & other, std::string const & filter,
...
void tidas::group::copy(group const & other, string const & filter,
                        backend_path const & loc) {
...
void tidas::group::read_field_astype(std::string const & field_name,
                                     ...
                                     tidas::data_type type, void * data) const {
    field check = schm_.field_get(field_name);
    if ((check.name != field_name) && (field_name != group_time_field)) {
        std::ostringstream o;
        o << "cannot read non-existent field " << field_name << " from group " <<
            loc_.path << "/" << loc_.name;
        TIDAS_THROW(o.str().c_str());
    }
    if (offset + ndata > size_) {
        ...
        (offset + ndata - 1) << " from group " << loc_.name << " (" << size_ <<
            " samples)";
        TIDAS_THROW(o.str().c_str());
    }
    if (loc_.type != backend_type::none) {
...
void tidas::group::write_field_astype(std::string const & field_name, index_type offset,
                                      index_type ndata, tidas::data_type type,
                                      void const * data) {
    field check = schm_.field_get(field_name);
    if ((check.name != field_name) && (field_name != group_time_field)) {
        std::ostringstream o;
        o << "cannot write non-existent field " << field_name << " from group " <<
            loc_.path << "/" << loc_.name;
        TIDAS_THROW(o.str().c_str());
    }
    if (offset + ndata > size_) {
        ...
        (offset + ndata - 1) << " to group " << loc_.name << " (" << size_ <<
            " samples)";
        TIDAS_THROW(o.str().c_str());
    }
    if (loc_.type != backend_type::none) {
...
void tidas::group::read_field(std::string const & field_name, index_type offset,
...
void tidas::group::write_field(std::string const & field_name, index_type offset,
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        if (data[i].size() > backend_string_size) {
            std::ostringstream o;
            o << "cannot write string \"" << data[i] << "\" to field " << field_name <<
                ".  Maximum data string length is " << backend_string_size;
            TIDAS_THROW(o.str().c_str());
        }
