...
void tidas::hdf5_helper_field_read_raw(backend_path const & loc,
                                       size_t type_indx, index_type offset,
                                       index_type ndata, data_type type,
                                       void * data) {
    ...
    std::string mpath = std::string("/") + group_hdf5_dataset_prefix +
                        std::string("_") + data_type_to_string(type);
    ...
    herr_t status = H5Sselect_hyperslab(file_dataspace, H5S_SELECT_SET, doff, NULL,
                                        dspan, NULL);
    ...
    status = H5Dread(dataset, mem_datatype, mem_dataspace, file_dataspace, H5P_DEFAULT,
                     data);
...
void tidas::hdf5_helper_field_write_raw(backend_path const & loc,
    ...
    status = H5Dwrite(dataset, mem_datatype, mem_dataspace, file_dataspace, H5P_DEFAULT,
                      data);
...
void tidas::hdf5_helper_field_read_str(backend_path const & loc, size_t type_indx,
...
void tidas::hdf5_helper_field_write_str(backend_path const & loc, size_t type_indx,
                                        index_type offset, index_type ndata,
                                        char * const * data) {
    hdf5_helper_field_write_raw(loc, type_indx, offset, ndata,
                                data_type::string,
                                static_cast <void const *> (data[0]));
    return;
}
...
group_backend_hdf5 & tidas::group_backend_hdf5::operator=(
    group_backend_hdf5 const & other) {
    if (this != &other) {}
    return *this;
}

string tidas::group_backend_hdf5::dict_meta() const {
    string meta = string("/") + group_hdf5_dataset_prefix + string("_") +
                  data_type_to_string(data_type::float64);
    return meta;
}
...
void tidas::group_backend_hdf5::read(backend_path const & loc, index_type & nsamp,
    ...
    if (ndims != 1) {
        ostringstream o;
        o << "HDF5 group range dataset " << fspath << ":" << mpath <<
    ...
    mpath = string("/") + group_hdf5_dataset_prefix + string("_") + data_type_to_string(
        data_type::float64);
    ...
    if (ndims != 2) {
        ostringstream o;
        o << "HDF5 group dataset " << fspath << ":" << mpath <<
            " has wrong dimensions (" << ndims << ")";
        TIDAS_THROW(o.str().c_str());
    }
    ...
    for (vector <data_type>::const_iterator it = othertypes.begin();
         it != othertypes.end(); ++it) {
        mpath = string("/") + group_hdf5_dataset_prefix + string("_") +
                data_type_to_string((*it));
        ...
        if (check) {
            ...
            if (nsamp_check != nsamp) {
                ostringstream o;
                o << "HDF5 group dataset " << fspath << ":" << mpath <<
                    " has a different number of samples than the time (" <<
                    nsamp_check <<
                    " != " << nsamp << ")";
                TIDAS_THROW(o.str().c_str());
            }
...
void tidas::group_backend_hdf5::write(backend_path const & loc,
                                      index_type const & nsamp, map <data_type,
                                                                     size_t> const & counts)
const {
#ifdef HAVE_HDF5
    ...
    for (map <data_type, size_t>::const_iterator it =
             counts.begin(); it != counts.end();
         ++it) {
        mpath = string("/") + group_hdf5_dataset_prefix + string("_") +
                data_type_to_string(it->first);
        ...
        dataset = H5Dcreate(file,
                            mpath.c_str(), datatype, dataspace, H5P_DEFAULT, dataprops,
                            H5P_DEFAULT);
...
void tidas::group_backend_hdf5::resize(backend_path const & loc,
                                       index_type const & nsamp) {
#ifdef HAVE_HDF5
    ...
    string mpath = string("/") + group_hdf5_dataset_prefix + string("_") +
                   data_type_to_string(data_type::float64);
...
void tidas::group_backend_hdf5::update_range(backend_path const & loc,
                                             time_type const & start,
                                             time_type const & stop) {
#ifdef HAVE_HDF5
...
void tidas::group_backend_hdf5::link(backend_path const & loc, link_type type,
...
void tidas::group_backend_hdf5::read_field(backend_path const & loc,
                                           string const & field_name, size_t type_indx,
                                           index_type offset, index_type ndata,
                                           int8_t * data) const {
...
void tidas::group_backend_hdf5::write_field(backend_path const & loc,
                                            string const & field_name, size_t type_indx,
                                            index_type offset, index_type ndata,
                                            int8_t const * data) {
