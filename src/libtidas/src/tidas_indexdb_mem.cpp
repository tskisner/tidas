...
indexdb_transaction & tidas::indexdb_transaction::operator=(
    indexdb_transaction const & other) {
    if (this != &other) {
        copy(other);
    }
    return *this;
...
void tidas::indexdb_transaction::print(ostream & out) const {
    ...
    switch (obj->type) {
        case indexdb_object_type::dict:
            ...
            for (auto d : dp->data) {
                out << "TR:     " << d.first << " = " << d.second << " (" <<
                    data_type_to_string(dp->types[d.first]) << ")" << endl;
            }
            break;
        ...
        case indexdb_object_type::schema:
            ...
            for (auto f : sp->fields) {
                out << "TR:     " << f.name << " : " << f.units << " : " <<
                    data_type_to_string(f.type) << endl;
            }
            break;
        ...
        case indexdb_object_type::group:
            ...
            out << "TR:     " << gp->nsamp << " samples, " << gp->start << " - " <<
                gp->stop << endl;
            out << "TR:     counts:" << endl;
            for (auto g : gp->counts) {
                out << "TR:       " << data_type_to_string(g.first) << " = " <<
                    g.second << endl;
            }
            break;
...
void tidas::indexdb_mem::ops_dict(backend_path loc, indexdb_op op, map <string,
...
void tidas::indexdb_mem::ops_schema(backend_path loc, indexdb_op op,
...
void tidas::indexdb_mem::ops_group(backend_path loc, indexdb_op op,
...
void tidas::indexdb_mem::ops_intervals(backend_path loc, indexdb_op op,
                                       size_t const & size) {
    string path = loc.path + path_sep + loc.name;
...
void tidas::indexdb_mem::add_dict(backend_path loc, map <string, string> const & data,
                                  map <string, data_type> const & types) {
    ops_dict(loc, indexdb_op::add, data, types);
    return;
}

void tidas::indexdb_mem::update_dict(backend_path loc, map <string,
                                                            string> const & data,
                                     map <string,
                                          data_type> const & types)
{
...
void tidas::indexdb_mem::add_group(backend_path loc, index_type const & nsamp,
...
void tidas::indexdb_mem::update_group(backend_path loc, index_type const & nsamp,
                                      time_type const & start, time_type const & stop,
                                      map <data_type, size_t> const & counts) {
...
bool tidas::indexdb_mem::query_dict(backend_path loc, map <string, string> & data,
...
bool tidas::indexdb_mem::query_group(backend_path loc, index_type & nsamp,
...
bool tidas::indexdb_mem::query_block(backend_path loc, vector <string> & child_blocks,
    ...
    if (data_block_.count(dpath) > 0) {
        map <string,
             indexdb_block>::const_iterator bit = data_block_.lower_bound(dpath);

        string dir;
        string base;

        while ((bit != data_block_.end()) &&
               (bit->first.compare(0, dpath.size(), dpath) == 0)) {
        ...
        map <string,
             indexdb_group>::const_iterator git = data_group_.lower_bound(grpdir);

        while ((git != data_group_.end()) &&
               (git->first.compare(0, grpdir.size(), grpdir) == 0)) {
        ...
        map <string,
             indexdb_intervals>::const_iterator it =
            data_intervals_.lower_bound(intdir);

        while ((it != data_intervals_.end()) &&
               (it->first.compare(0, intdir.size(), intdir) == 0)) {
    ...
    for (auto tr : trans) {
        switch (tr.op) {
            case indexdb_op::add:
                ...
                switch (tr.obj->type) {
                    case indexdb_object_type::dict:
                        data_dict_[tr.obj->path] =
                            *(dynamic_cast <indexdb_dict *> (tr.obj.get()));
                        break;

                    case indexdb_object_type::schema:
                        data_schema_[tr.obj->path] =
                            *(dynamic_cast <indexdb_schema *> (tr.obj.get()));
                        break;

                    case indexdb_object_type::group:
                        data_group_[tr.obj->path] =
                            *(dynamic_cast <indexdb_group *> (tr.obj.get()));
                        break;

                    case indexdb_object_type::intervals:
                        data_intervals_[tr.obj->path] =
                            *(dynamic_cast <indexdb_intervals *> (tr.obj.get()));
                        break;

                    case indexdb_object_type::block:
                        data_block_[tr.obj->path] =
                            *(dynamic_cast <indexdb_block *> (tr.obj.get()));
                        break;
