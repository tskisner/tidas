...
tidas::field::field(std::string const & fname, data_type ftype,
                    std::string const & funits) {
    type = ftype;
    name = fname;
    units = funits;
}
...
tidas::schema::schema(schema const & other, std::string const & filter,
...
void tidas::schema::copy(schema const & other, string const & filter,
                         backend_path const & loc) {
    // extract filters
