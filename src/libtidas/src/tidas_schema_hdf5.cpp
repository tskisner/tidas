...
schema_backend_hdf5 & tidas::schema_backend_hdf5::operator=(
    schema_backend_hdf5 const & other) {
    if (this != &other) {}
    return *this;
}
...
void tidas::schema_backend_hdf5::read(backend_path const & loc, field_list & fields) {
    ...
    if (ndims != 2) {
        ostringstream o;
        o << "HDF5 schema dataset " << fspath << ":" << loc.meta <<
    ...
    if (dims[1] != 3) {
        ostringstream o;
        o << "HDF5 schema dataset " << fspath << ":" << loc.meta <<
            " has wrong second dimension (" << dims[1] << " != " << 3 << ")";
        TIDAS_THROW(o.str().c_str());
    }
...
void tidas::schema_backend_hdf5::write(backend_path const & loc,
    ...
    hid_t dataset = H5Dcreate(file,
                              loc.meta.c_str(), datatype, dataspace, H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT);
