...
tidas::volume::volume(string const & path, backend_type type, compression_type comp,
    ...
    if (fs_stat(relpath.c_str()) >= 0) {
        ostringstream o;
        o << "cannot create volume \"" << relpath <<
...
tidas::volume::volume(volume const & other, std::string const & filter,
...
void tidas::volume::copy(volume const & other, string const & filter,
                         backend_path const & loc) {
    ...
    if (loc_ != other.loc_) {
        ...
        if (loc_.path != "") {
            if (fs_stat(loc_.path.c_str()) >= 0) {
                ostringstream o;
                o << "cannot create volume \"" << loc_.path <<
                    "\", a file or directory already exists";
                TIDAS_THROW(o.str().c_str());
            }
...
void tidas::volume::link(std::string const & path, link_type const & ltype,
...
void tidas::volume::duplicate(std::string const & path, backend_type type,
