...
void indexdb_setup_block(tidas::indexdb * idx, backend_path const & loc) {
    ...
    for (size_t n = 0; n < NINTERVAL; ++n) {
        ...
        intrloc.path = loc.path + path_sep + loc.name + path_sep +
...
TEST(indexdbtest, pathsearch) {
    ...
    for (auto b : blk) {
        ...
        while ((it != store.end()) &&
               (it->first.compare(0, bpath.size(), bpath) == 0)) {
            ...
            if (it->first.size() > bpath.size()) {
                ...
                // cout << "D2:    check for " << path_sep << " in " << it->first.substr
                // ( off ) << endl;
